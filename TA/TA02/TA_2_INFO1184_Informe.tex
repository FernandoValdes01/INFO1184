...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.55\textwidth]{fig_01_distribucion_y.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.60\textwidth]{fig_02_edad_balance.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.65\textwidth]{fig_03_boxplot_balance_job.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.45\textwidth]{fig_04_correlaciones.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth]{fig_05_dendrograma_ward.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.80\textwidth]{fig_06_elbow_silhouette.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.60\textwidth]{fig_07_clusters_pca.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.65\textwidth]{fig_08_perfiles_clusters.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.60\textwidth]{fig_09_silueta_final.png}
...
Los resultados del agrupamiento jerárquico revelan que la base de clientes del banco no es homogénea, sino que presenta segmentos diferenciados con características propias. La identificación de 4 clusters mediante el método Ward.D2 permitió descubrir perfiles claramente interpretables: un grupo mayoritario de clientes estándar (Cluster~1, 71,1\%), un segmento con préstamos personales (Cluster~2), clientes con historial de contacto previo (Cluster~3) y un grupo de alto riesgo en mora (Cluster~4).

La metodología CRISP-DM proporcionó un marco estructurado que facilitó la organización del trabajo. La fase de comprensión empresarial fue esencial para orientar el análisis hacia objetivos accionables, mientras que la comprensión de los datos reveló patrones como el desbalance de la variable objetivo (88,3\% no suscribieron) y la predominancia de clientes no contactados previamente (81,7\% con pdays~=~$-1$).

El coeficiente de silueta promedio de 0,2302 indica una estructura de clusters moderada, lo cual es esperable en datos reales de marketing con alta variabilidad interna. Si bien $k = 3$ presentó una silueta ligeramente superior (0,2654), se optó por $k = 4$ por su mayor riqueza interpretativa al separar el segmento de clientes en mora, información de alto valor para la gestión de riesgo.

Una limitación importante es el uso de muestreo (2.000 de 45.211 registros) debido a restricciones computacionales del clustering jerárquico, cuya complejidad es $O(n^2)$ en memoria. Si bien la muestra es representativa gracias a la semilla fija, los perfiles podrían variar ligeramente con el dataset completo. Adicionalmente, la codificación numérica ordinal de la variable educación implica asumir equidistancia entre niveles.

Desde la perspectiva de negocio, los segmentos identificados permiten diseñar estrategias diferenciadas: el Cluster~1 (clientes estándar) es el público objetivo natural para campañas masivas de depósitos a plazo; el Cluster~2 (con préstamo activo) podría beneficiarse de ofertas de consolidación de deuda; el Cluster~3 (contacto previo) ya tiene familiaridad con la campaña y podría responder mejor a un seguimiento personalizado; y el Cluster~4 (mora) debe ser excluido de campañas de captación y derivado a programas de recuperación crediticia.

% ==================== CONCLUSIONES ====================
\section{Conclusiones}

\textbf{Integrante 1:} La aplicación de la metodología CRISP-DM demostró ser un marco efectivo para estructurar el proceso de análisis de datos desde la comprensión empresarial hasta el modelamiento. La segmentación mediante clustering jerárquico aglomerativo con Ward.D2 permitió identificar 4 grupos diferenciados de clientes ---estándar (71,1\%), con préstamo personal, con historial de contacto y en mora--- validados por el coeficiente de silueta (0,2302) y el método del codo, lo que confirma la utilidad de esta técnica para descubrir estructura en datos de marketing bancario.

\textbf{Integrante 2:} El análisis exploratorio reveló características fundamentales del dataset, como el fuerte desbalance en la variable objetivo (88,3\% no suscribieron), la alta variabilidad del balance (DE~=~3.045~EUR) y que el 81,7\% de los clientes no habían sido contactados previamente. La preparación de datos, incluyendo la codificación numérica y la estandarización \textit{z-score}, fue un paso crítico para garantizar la calidad del agrupamiento. Los perfiles obtenidos ---en particular la identificación de un segmento en mora con balance negativo--- proporcionan información accionable tanto para la estrategia comercial como para la gestión de riesgo del banco.

\textbf{Integrante 3:} La implementación en R Project facilitó el análisis completo, desde la carga de datos hasta la visualización de resultados. El dendrograma y la proyección PCA ofrecen perspectivas complementarias para interpretar los clusters. El segmento de clientes con contacto previo (Cluster~3, pdays~$\approx$~237 días) sugiere que el historial de interacción es un factor diferenciador relevante. Como trabajo futuro, sería valioso aplicar técnicas como $k$-means sobre el dataset completo y comparar los segmentos obtenidos, así como integrar variables categóricas mediante distancias de Gower para enriquecer el análisis.
